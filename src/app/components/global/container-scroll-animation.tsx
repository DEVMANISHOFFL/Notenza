...
export const ContainerScroll = ({
    ...
    const [scaleRange, setScaleRange] = useState<[number, number]>([1.05, 1.01]) // Prevents final zoom-out

    useEffect(() => {
        const checkMobile = () => {
            const mobile = window.innerWidth <= 768
            setIsMobile(mobile)
            setScaleRange(mobile ? [0.7, 1] : [1.05, 1.01]) // Slightly above 1 to prevent zoom-out
        }
        checkMobile()
    ...
    const rotate = useTransform(scrollYProgress, [0, 1], [20, 0]) // Prevents abrupt flattening
